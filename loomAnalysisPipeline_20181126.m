...
%in window [RJ]
loomwin = movsum(raw_loom, [0 199]); %calc sliding window of 200 frames after the current frame that "real" stimuli must be on for
loomon = find(raw_loom); %locate all frames where stimulus presentation was on
loomstart=[];
for cloom = 1:length(loomon)
    if cloom==1
        if loomon(cloom)==1 && loomwin(loomon(cloom))==200 %if loom starts at 1 and is on for at least 200 frames
...
% [RJ] Does this correctly deal with stimulus jitter?  ie should the stim
% start be the start before the few frames off, or after the few frames
% off?
...
for mm = 1:size(locsz,1)
    ...
    tmp_swim_bouts(mm,17) = tmp_MaxAngVel;
        % tmp_MaxAngVelIdx gives the position index within the bout of the max ang velocity
    ...
    %tmp_swim_bouts(mm,20) = (nansum(islocalmax(tmp_delta_ori_filtered2((locsz(mm,2):locsz(mm,1))),'MinSeparation',2)))/((locsz(mm,2)-locsz(mm,1))*1.33)/1000;
        %I think that locsz is not the variable needed here - maybe tmp_swim_bouts is what to use? [RJ]
end
