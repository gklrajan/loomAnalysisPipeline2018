...
%extracts out the booleans for stimuli 11/23/18 - Need to adjust # frames
%in window [RJ]
loomwin = movsum(raw_loom, [0 499]); %calc sliding window of 500 frames that "real" stimuli must be on for
loomon = find(raw_loom); %locate all frames where stimulus was on
loomstart=[];
for cloom = 1:length(loomon)
    if cloom==1
    if loomon(cloom)==1 && loomwin(loomon(cloom))==500 %if loom starts at 1 and is on for at least 500 frames
        loomstart = loomon(cloom); %store loomon(cloom) as the first value in loomstart
    end
    elseif cloom>1
    if (loomon(cloom)-loomon(cloom-1))>1 && loomwin(loomon(cloom))==500 %if there's a 2sec stretch with no stim before the stim started
...
escapeLocs=locsz(pks>30); % if velocity is >30mm/s
escapePks=pks(pks>30);

%% new - 20180829 - verify if locs are associated with looms and operate acc

% create a draft RT matrix - (all looms - all escapes in tmp_difftap)
...
klm=1;
for sss=1:size(tmp_diffTap,2) %row counter is sss to go through the stimuli
    for aaa=1:size(tmp_diffTap,1) %column counter is aaa to go through the escape bouts
...
fast_locs=locsz(idx_corrTap2); %in case we want to only go through the escape bouts --> use this
...
for qq = 1:size(locsz)
    ...
    if (locsz(qq) - loomEscape(ff).boutAnalysis.boutLength <=0)... % get rid of any imcomplete bout in the begining
                ||(locsz(qq) + loomEscape(ff).boutAnalysis.boutLength >=size(tmp_vel_fB,1))... % incomplete bout in the end
...
%13 Reaction Time (lag in response)
%14 local bend minima [RJ]
%15 local bend maxima [RJ]
%16 total half bends [RJ]
%17 Maximum Angular Velocity (instantaneous) of the bout [RJ]
%18 Peak Turn Angle of the bout [RJ]
%19 Direction of the largest turn [RJ]
%20 Tail Beat Frequency associated velocity? [RJ]

locsz(isnan(locsz)) = []; %imp - esle, error w/ for loop - sunbscript indices must be real integers or logicals

tmp_swim_bouts = zeros(size(locsz,1),20); %Changed to 20 parameters [11-20-18 RJ]
...
for mm = 1:size(locsz,1)
    ...
    if sum(locsz(mm)==idx_escape)>=1
        nn=find(locsz(mm)==idx_escape);
    tmp_swim_bouts(mm,13) = (locsz(nn)-idx_tap(nn))*frame_length_calc_ms; %if there is an error --> error in tap/escape ID
    ...
    %tail half bends - (Calc'd without using islocalmin/max)
    ...
    [locExtremes, tmp_ExtremeIdx] = findpeaks(abs(tmp_delta_ori_filtered2((tmp_swim_bouts(mm,1)):(tmp_swim_bouts(mm,2)))),'MinPeakDistance',10);
        % finds +/- peaks of inst ang velocity
    [locInflect, tmp_InflectIdx] = findpeaks(-abs(tmp_delta_ori_filtered2((tmp_swim_bouts(mm,1)):(tmp_swim_bouts(mm,2)))),'MinPeakDistance',10);
        % finds inflection points of inst ang vel, which should be the
        % points of max turn angles for each bend
    [tmp_MaxAngVel, tmp_MaxAngVelIdx] = max(abs(tmp_delta_ori_filtered2(tmp_swim_bouts(mm,1):tmp_swim_bouts(mm,2))));
    tmp_swim_bouts(mm,17) = tmp_MaxAngVel;
        % tmp_MaxAngVelIdx gives the position index of the max ang velocity
    % Find Peak Ang Vel value in locExtremes, then find the local abs(min)
    % values surrounding that peak (use the local inflection points)
    ...
    if tmp_delta_ori_filtered2(tmp_MaxAngVelIdx)>0
        tmp_swim_bouts(mm,19) = 1; % Max head turn is to the Right
    else tmp_swim_bouts(mm,19) = -1; %Max head turn is to the Left
    end
    
    %TBF
    %tmp_swim_bouts(mm,20) = (nansum(islocalmax(tmp_delta_ori_filtered2((locsz(mm,2):locsz(mm,1))),'MinSeparation',2)))/((locsz(mm,2)-locsz(mm,1))*1.33)/1000;
        %I think that locsz is not the variable needed here? [RJ]
end
